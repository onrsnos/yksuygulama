...
export default class tarih extends Component{
  ...
  firebase.database().ref('matematik').on('value', (data) => {
            console.log(data.toJSON());
        })

        // To Await 5 seconds to insert a new user
        setTimeout(() => {
            firebase.database().ref('matematik/001').set(
                {
        ...
        // To Remove a user
        firebase.database().ref('matematik/001').remove();
  ...
  render(){
    return(
      ...
      <Button title="İleri"> {sorusayisi++} </Button>
